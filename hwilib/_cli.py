...
def get_parser() -> HWIArgumentParser:
    ...
    signtx_parser.add_argument('psbt', help='The Partially Signed Fujicoin Transaction to sign')
    ...
    getkeypool_parser = subparsers.add_parser('getkeypool', help='Get JSON array of keys that can be imported to Fujicoin Core with importmulti')
    ...
    kp_type_group.add_argument('--all', action='store_true', help='Generate addresses for all standard address types (default paths: ``m/{44,49,84}h/75h/0h/[0,1]/*)``')
    getkeypool_parser.add_argument('--account', help='BIP43 account', type=int, default=0)
    getkeypool_parser.add_argument('--path', help='Derivation path, default follows BIP43 convention, e.g. ``m/84h/75h/0h/1/*`` with --addr-type wpkh --internal. If this argument and --internal is not given, both internal and external keypools will be returned.')
    ...
    group.add_argument('--desc', help='Output Descriptor. E.g. wpkh([00000000/84h/75h/0h]xpub.../0/0), where 00000000 must match --fingerprint and xpub can be obtained with getxpub. See doc/descriptors.md in Fujicoin Core')
    group.add_argument('--path', help='The BIP 32 derivation path of the key embedded in the address, default follows BIP43 convention, e.g. ``m/84h/75h/0h/1/*``')
